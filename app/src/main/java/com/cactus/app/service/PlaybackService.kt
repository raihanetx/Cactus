...
class PlaybackService : Service() {
    ...
    override fun onStartCommand(intent: Intent?, flags: Int, startId: Int): Int {
        ...
        intent?.action?.let { action -> if (action == ACTION_TOGGLE || action == ACTION_SEEK_BACK || action == ACTION_SEEK_FORWARD || action == ACTION_STOP) { playerHolder.handleAction(action); updateNotification() } }
    ...
    private fun updateMediaSession() {
        ...
        val metadata = MediaMetadataCompat.Builder().putString(MediaMetadataCompat.METADATA_KEY_TITLE, s.title.ifBlank { "Cactus" }).putString(MediaMetadataCompat.METADATA_KEY_ARTIST, "Cactus").putLong(MediaMetadataCompat.METADATA_KEY_DURATION, s.durationMs).build()
    ...
    private fun buildNotification(): Notification {
        ...
        fun RemoteViews.populate() {
            setTextViewText(R.id.track_title, s.title.ifBlank { "Cactus" })
        ...
        val collapsedView = RemoteViews(packageName, R.layout.notification_glass_collapsed).apply { populate(); setOnClickPendingIntent(R.id.notification_root_collapsed, contentIntent) }
        val expandedView = RemoteViews(packageName, R.layout.notification_glass_expanded).apply { populate(); setOnClickPendingIntent(R.id.notification_root, contentIntent) }
        return NotificationCompat.Builder(this, CHANNEL_ID)
            .setSmallIcon(R.drawable.ic_music_note)
            .setContentTitle(s.title.ifBlank { "Cactus" })
            .setContentIntent(contentIntent).setOngoing(s.isPlaying).setShowWhen(false)
            .setVisibility(NotificationCompat.VISIBILITY_SECRET).setPriority(NotificationCompat.PRIORITY_LOW)
    ...
    private fun createNotificationChannel() {
        val channel = NotificationChannel(CHANNEL_ID, "Playback", NotificationManager.IMPORTANCE_LOW).apply { setShowBadge(false); enableLights(false); enableVibration(false); lockscreenVisibility = Notification.VISIBILITY_SECRET }
